...
class AudioReceiver:
    ...
    def recognized_cb(self, evt: speechsdk.translation.TranslationRecognitionEventArgs):
        ...
        if translation_recognition_result.reason == speechsdk.ResultReason.TranslatedSpeech:
            if self.__auto_translate:
                self.__transcribe_op('\tlanguage={}'.format(evt.result.properties[speechsdk.PropertyId.SpeechServiceConnection_AutoDetectSourceLanguageResult]))
    ...
    def update_lang(self,src, des):
        ...
        if self.__auto_translate:
            ...
            self._speech_translation_config.set_property(property_id=speechsdk.PropertyId.SpeechServiceConnection_LanguageIdMode, value='Continuous')
            auto_detect_source_language_config = speechsdk.languageconfig.AutoDetectSourceLanguageConfig(languages=list(auto_translate_dict.keys()))
            self._translation_recognizer = speechsdk.translation.TranslationRecognizer(
                translation_config=self._speech_translation_config, audio_config=self._audio_config,auto_detect_source_language_config=auto_detect_source_language_config)
        else:
            self._speech_translation_config = speechsdk.translation.SpeechTranslationConfig(subscription=self.__speech_key,
    ...
    def conversation_transcriber_transcribed_cb(self,evt: speechsdk.SpeechRecognitionEventArgs):
        ...
        if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
            ...
            self.__transcribe_cb(evt.result.speaker_id)
    ...
    def __client_connection_mtdata(self, connection, address):
        while self.__running:
            ...
            if not (self._speech_translation_config is None or
                ...
                print('lang- {} ; new_lang - {}, target_lang {} ; '.format(language,self._speech_translation_config.speech_recognition_language,self._target_language))
